...
class Kappa:
    ...
    def __init__(self):
        ...
        self.id_sep = (
            ".",
            ".",
        )  # not any of '(?:[_~][a-zA-Z0-9_~+-]+|[a-zA-Z][a-zA-Z0-9_~+-]*)'
        ...
        self.binding_state_re = re.compile(
            r"^"
            + r"(?:\.|_|#|\d+)"
            + r"|(?:"
            + self.symbols
        ...
        # to dissect agents
        self.agent_re = re.compile(
            r"^" + sID_re + agent_name_re + agent_interface_re + r"$"
        )
        # to find all agents (hence groups are non-capturing), before dissecting them
        self.agents_re = re.compile(
    ...
    def parser(self, kappa_string: str, start_label: int = 1) -> dict[str]:
        ...
        expression = re.sub(
            r"\s+|\t+|\n+", " ", kappa_string
        )  # remove line breaks and white matter
        # capture all agents
    ...
    def parse_site(self, site_expression: str) -> tuple:
        ...
        if match.group(
            2
        ):  # the modification state; it may be absent, so we need to check
            ...
            while stack:
                ...
                if agent1["back"]:
                    ...
                    for j in range(agent1["pos"] + 1, len(agents)):
                        ...
                        if [other_agent, other_site, agent_type, site_name] in agent2[
                            "ports"
                        ]:
                            ...
                            # annihilate the matching port
                            agent2["ports"].remove(
                                [other_agent, other_site, agent_type, site_name]
                            )
                            # also annihilate the matched port
...
def KappaComplex(
    ...
    _kappa = Kappa()
    molecule = KappaMolecule(
        agents=_kappa.parser(k_expression),
        count=count,
        id_shift=id_shift,
        system=system,
        sig=signature,
...
class KappaExpression:
    ...
    def initializeExpression(self, canon: bool = False, nav: bool = False) -> None:
        ...
        self.label_counter = int(
            get_identifier(next(reversed(self.agents)), delimiters=self.id_sep)[1]
        )
        # get the composition
        ...
        if nav:
            ...
            for at in self.composition:
                self.type_slice.extend(
                    [
                        [
                            name
                            for name in self.agents
                            if self.agents[name]["info"]["type"] == at
                        ]
                    ]
                )
                ...
                if link != ".":
                    if is_number(link):
                        ...
                        if link in bonds:
                            ...
                            # stubbify
                            self.agents[name1]["iface"][site1]["bond"] = "".join(
                                [name, self.bond_sep, site]
                            )
                            self.agents[name]["iface"][site]["bond"] = "".join(
                                [name1, self.bond_sep, site1]
                            )
                        else:
                    ...
                    b = sorted(
                        [(name1, site1), (name, site)],
                        key=lambda x: (alphanum_key(x[0]), alphanum_key(x[1])),
                    )
                    b = tuple(b)
                    # collect unique bonds
                    self.bonds[b] = (
                        1  # just an indicator; we are collecting unique keys (bonds)
                    )
                else:
                    self.n_free_sites += 1
    ...
    def stubbify_bonds_with_shift(self, id_shift: int = 0, remap=None) -> None:
        ...
        for name in self.agents:
            ...
            new_agents[new_name]["local_view"] = self.agents[name][
                "local_view"
            ]  # shift-independent
            degree = 0
            ...
            for site in self.agents[name]["iface"]:
                ...
                new_agents[new_name]["iface"][site]["state"] = self.agents[name][
                    "iface"
                ][site]["state"]
                # may be overwritten below
                new_agents[new_name]["iface"][site]["bond"] = self.agents[name][
                    "iface"
                ][site]["bond"]
                link = self.agents[name]["iface"][site]["bond"]
                if link != ".":
                    if is_number(link):
                        ...
                        if link in bonds:
                            ...
                            new_name1 = add_identifier(
                                self.agents[name1]["info"]["type"], new_id1
                            )
                            new_agents[new_name1]["iface"][site1]["bond"] = "".join(
                                [new_name, self.bond_sep, site]
                            )
                            new_agents[new_name]["iface"][site]["bond"] = "".join(
                                [new_name1, self.bond_sep, site1]
                            )
                        else:
                            bonds[link] = (name, site)
                            continue
                    ...
                    (t1, l1, s1), (t2, l2, s2) = sorted(
                        [(n1, int(new_id1), site1), (n, int(new_id), site)]
                    )
                    b = (add_identifier(t1, str(l1)), s1), (
                        add_identifier(t2, str(l2)),
                        s2,
                    )
                    # collect unique bonds
                    self.bonds[b] = (
    ...
    def make_local_view_lists(self) -> None:
        ...
        for name in self.agents:
            ...
            if lv in self.local_views:
                self.local_views[lv][
                    name
                ] = 1  # just an indicator dict for fast search and deletion
            else:
    ...
    def traverse(self, node) -> str:
        ...
        while stack:
            ...
            if current not in discovered:
                spanning.add(
                    tuple(sorted((parent[current], current)))
                )  # sort, since graph is undirected
                ...
                discovered.add(
                    current
                )  # for faster search (or else we could use 'traversal')
        ...
        for n, back_list in traversal:
            ...
            if back_list:
                for (
                    ...
                    back_list
                ):  # sorting is necessary to achieve a deterministic result
    ...
    def make_adjacency_lists(self) -> None:
        ...
        for name1 in self.agents:
            ...
            adjacency = [
                iface[s1]["bond"].split(self.bond_sep)[0]
                for s1 in iface
                if iface[s1]["bond"] not in {".", "#", "_"}
            ]
    ...
    def make_navigation_list(self) -> None:
        ...
        for (a1, s1), (
            a2,
            s2,
        ) in self.bonds:  # names a1 and a2 in bonds have 'id' attached
    ...
    def _remap_ids(self, remapping: dict) -> None:
        ...
        for name1 in self.agents:
            ...
            for site1 in self.agents[name1]["iface"]:
                ...
                renamed[new_name1]["iface"][site1]["state"] = self.agents[name1][
                    "iface"
                ][site1]["state"]
                if self.bond_sep in self.agents[name1]["iface"][site1]["bond"]:
                    # agent2 is name.old_id
                    agent2, site2 = self.agents[name1]["iface"][site1]["bond"].split(
                        self.bond_sep
                    )
                    type2, id2 = get_identifier(agent2)
                    new_id2 = remapping[id2]
                    new_name2 = add_identifier(type2, new_id2)
                    renamed[new_name1]["iface"][site1]["bond"] = (
                        new_name2 + self.bond_sep + site2
                    )
                    # sort
                    (t1, l1, s1), (t2, l2, s2) = sorted(
                        [(type1, int(new_id1), site1), (type2, int(new_id2), site2)]
                    )
                    b = (add_identifier(t1, str(l1)), s1), (
                        add_identifier(t2, str(l2)),
                        s2,
                    ...
                    renamed[new_name1]["iface"][site1]["bond"] = self.agents[name1][
                        "iface"
                    ][site1]["bond"]
    ...
    def get_composition(self) -> None:
        ...
        self.composition = {
            k: v for k, v in sorted(comp.items(), key=lambda item: (item[1], item[0]))
        }
    ...
    def show(self, internal: bool = False, label: bool = False, wrap=-1) -> str:
        ...
        if internal:
            ...
            if wrap > 0:
                info = pprint.pformat(
                    self.kappa_expression(label=label),
                    indent=0,
                    width=wrap,
                    compact=False,
                )
...
class KappaMolecule(KappaExpression):
    ...
    def __init__(
        ...
        super().__init__(
            agents=agents, id_shift=id_shift, nav=nav, canon=canon, init=False
        )
        ...
        self.bond_type = (
            {}
        )  # number of bonds of a given type (w/o labels, as in self.bonds)
        self.bond_list = {}  # lists of bonds indexed by bond type
        self.free_site_list_idx = {}
        self.bond_list_idx = {}
        self.agent_self_binding = (
            {}
        )  # excluded intra-agent binding opportunities; indexed by bond type
        ...
        self.unbinding = (
            {}
        )  # propensity of internal bond dissociation; indexed by bond type
    ...
    def internal_reactivity(self) -> None:
        for bt in self.signature.bond_types:
            ...
            if Xs == Yp:  # symmetry correction
                self.binding[bt] = (self.free_site[Xs] * (self.free_site[Xs] - 1)) / 2.0
            else:
                self.binding[bt] = (
                    self.free_site[Xs] * self.free_site[Yp]
                    - self.agent_self_binding[bt]
                )
            ...
            for site in self.agents[name]["iface"]:
                ...
                if link != ".":
                    ...
                    if self.signature:
                        ...
                        self.bond_list_idx[bt][b] = self.bond_type[
                            bt
                        ]  # (ab)used as a counter
                        self.bond_type[bt] += 1
                else:
                    ...
                    if self.signature:
                        ...
                        agent_free_site_types.add(
                            st
                        )  # a local agent-specific set, used below
                        self.free_site_list[st].append((name, site))
                        self.free_site_list_idx[st][(name, site)] = self.free_site[
                            st
                        ]  # (ab)used as a counter
                        self.free_site[st] += 1
            if self.signature:
                ...
                for bt in self.signature.bond_types:
                    ...
                    if st1 != st2:
                        if (
                            st1 in agent_free_site_types
                            and st2 in agent_free_site_types
                        ):
            ...
            for site in self.agents[name]["iface"]:
                ...
                if link != ".":
                    ...
                    if self.signature:
                        ...
                        self.free_site_list_idx[st][(new_name, site)] = self.free_site[
                            st
                        ]  # (ab)used as a counter
                        self.free_site[st] += 1
            if self.signature:
        ...
        if self.signature:
            ...
            for bt in self.signature.bond_types:
                ...
                for b1, b2 in self.bond_list[bt]:
                    ...
                    new_id2 = remapping[id2]
                    (t1, l1, s1), (t2, l2, s2) = sorted(
                        [(t1, int(new_id1), s1), (t2, int(new_id2), s2)]
                    )
                    b = (add_identifier(t1, str(l1)), s1), (
                        add_identifier(t2, str(l2)),
                        s2,
