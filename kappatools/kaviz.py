...
class Canvas:
    def __init__(self, rows=1, cols=1, size=(10, 10), name=None):
        ...
        self.figure, self.axes = plt.subplots(
            nrows=rows, ncols=cols, figsize=size, squeeze=False, frameon=False
        )
...
class Renderer:
    def __init__(self, komplex, prog="neato", node_info=False):
        ...
        for node in self.nx_graph.nodes:
            self.nx_options["node_color"] += [
                self.nx_palette[self.type_color[self.nx_graph.nodes[node]["type"]]]
            ]
        self.legend_colors = [
            f"{self.nx_palette[self.type_color[n]]}" for n in self.type_color.keys()
        ]
    ...
    def refresh(self, canvas=None, panel=(1, 1), figure_size=(6, 6)):
        ...
        nx.draw_networkx(
            self.nx_graph, pos=self.positions, ax=self.ax, **self.nx_options
        )
    ...
    def render(
        ...
        if labels == "type":
            self.nx_options["labels"] = {
                node: self.nx_graph.nodes[node]["type"] for node in self.nx_graph.nodes
            }
        elif labels == "short":
            self.nx_options["labels"] = {
                node: self.nx_graph.nodes[node]["id"] for node in self.nx_graph.nodes
            }
        elif labels == "full":
            self.nx_options["labels"] = {
                node: self.nx_graph.nodes[node]["type"]
                + self.nx_graph.nodes[node]["id"]
                for node in self.nx_graph.nodes
            }
        else:
    ...
    def delete_edge_lists(self, edge_list=[]):
        ...
        for list_of_edges in edge_list:
            untouched_edges = untouched_edges - set(
                [frozenset(e) for e in list_of_edges]
            )
    ...
    def delete_node_list(self, node_list=[]):
        untouched_nodes = set(n for n in self.nx_graph.nodes()) - set(
        ...
        edges = set([frozenset(e) for e in self.nx_graph.edges()]) - set(
            [frozenset(e) for e in e_to_delete]
        )
...
def show_ranked_complexes(
    snapshot, canvas, sort="size", cutoff=3, cols=3, rows=1, prog="neato"
):
    """
...
if __name__ == "__main__":
    ...
    line = (
        "A(l[19] r[.] p[2]), A(l[53] r[19] p[42]), A(l[37] r[53] p[45]), A(l[.] r[37] p[29]), P(a1[3] a2[51] a3["
